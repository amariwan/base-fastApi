from __future__ import annotations

"""Schemas returned by the healthcheck endpoints."""

from typing import Literal

from pydantic import BaseModel, ConfigDict, Field, StrictBool, StrictInt, StrictStr

type HealthCheckStatus = Literal["ok", "skipped", "fail"]
type HealthStatus = Literal["ok", "fail"]
type RuntimeConfigScalar = None | bool | int | float | str
type RuntimeConfigValue = RuntimeConfigScalar | list[str]
type RuntimeConfigSection = dict[str, RuntimeConfigValue]
type ServiceRuntimeConfig = dict[str, RuntimeConfigSection]


class HealthSettingsApp(BaseModel):
    """Minimal application configuration exposed via /health."""

    model_config = ConfigDict(populate_by_name=True, extra="forbid")

    log_level: StrictStr = Field(..., alias="LOG_LEVEL")
    test_mode: StrictBool = Field(..., alias="TEST_MODE")


class HealthSettingsDB(BaseModel):
    """Minimal database configuration exposed via /health."""

    model_config = ConfigDict(populate_by_name=True, extra="forbid")

    port: StrictInt = Field(..., alias="DB_PORT", ge=0)
    database: StrictStr = Field(..., alias="DB_DATABASE")


class HealthConfig(BaseModel):
    """Combined app+db view for diagnostics."""

    model_config = ConfigDict(populate_by_name=True, extra="forbid")

    app: HealthSettingsApp = Field(..., alias="APP_SETTINGS")
    db: HealthSettingsDB = Field(..., alias="DB_SETTINGS")


class HealthCheckResult(BaseModel):
    """Status information for one system check."""

    model_config = ConfigDict(extra="forbid")

    status: HealthCheckStatus
    message: StrictStr
    details: RuntimeConfigSection = Field(default_factory=dict)


class HealthResponse(BaseModel):
    """Payload returned by /health."""

    model_config = ConfigDict(extra="forbid")

    status: HealthStatus = "ok"
    checks: dict[str, HealthCheckResult] = Field(default_factory=dict)
    config: HealthConfig | None = None


class HealthLiveResponse(BaseModel):
    """Payload returned by /health/live."""

    model_config = ConfigDict(extra="forbid")

    status: Literal["ok"] = "ok"
    checks: dict[str, HealthCheckResult] = Field(default_factory=dict)


class WhoAmIResponse(BaseModel):
    """Normalized user identity response."""

    sub: StrictStr | None = None
    name: StrictStr | None = None
    email: StrictStr | None = None
    preferred_username: StrictStr | None = None
    organisation: StrictStr | None = None
    roles: list[StrictStr] = Field(default_factory=list)


class RuntimeConfigResponse(BaseModel):
    """Redacted runtime configuration for diagnostics."""

    model_config = ConfigDict(extra="forbid")

    app: RuntimeConfigSection
    db: RuntimeConfigSection
    auth: RuntimeConfigSection
    roles: RuntimeConfigSection
    storage: RuntimeConfigSection
    services: ServiceRuntimeConfig
    redacted_fields: list[StrictStr] = Field(default_factory=list)


__all__ = [
    "HealthResponse",
    "HealthLiveResponse",
    "WhoAmIResponse",
    "HealthConfig",
    "HealthSettingsApp",
    "HealthSettingsDB",
    "HealthCheckResult",
    "HealthCheckStatus",
    "HealthStatus",
    "RuntimeConfigScalar",
    "RuntimeConfigValue",
    "RuntimeConfigSection",
    "ServiceRuntimeConfig",
    "RuntimeConfigResponse",
]
