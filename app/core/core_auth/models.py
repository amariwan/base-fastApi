...
class UserClaims(BaseModel):
    ...
    groups: list[StrictStr] = Field(default_factory=list)
    ...
    organisation: StrictStr | None = Field(default=None, max_length=200)
    ...
    def normalize_groups(cls, groups: list[str]) -> list[str]:
        """Deduplicate and lowercase group entries while preserving order."""
        seen: set[str] = set()
        normalized: list[str] = []
        for group in groups:
